describe('template spec', () => {
  it('passes', () => {
    ...
    cy.get('[data-testid="cart-modal-btn"] span.flex').click();
    cy.get('#username [name="email"]').click();
    ...
    cy.get('#autocomplete-0-input').type('GROUPEDDD');
    cy.get('[href="/shop/brand/test-1/grouped"]').click();
    cy.get('[data-testid="add-to-cart-cta"] span.block').click();
    cy.get('#autocomplete-0-label').click();
    cy.get('#autocomplete-0-input').type('STARTER REFIL - BREAD & CHEESE');
    ...
    cy.get('tr:nth-of-type(2) [data-testid="brand-header"] span.block').contains('Groupeddd');
    cy.get('tr:nth-of-type(3) [data-testid="brand-header"] span.block').contains('Starter Refil - bread & cheese');
